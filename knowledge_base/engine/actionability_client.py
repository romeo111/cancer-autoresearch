...
# ── Snapshot client (Phase 2 placeholder) ────────────────────────────────


class SnapshotActionabilityClient:
    """Placeholder for the upcoming CIViC-snapshot reader.

    Phase 2 will implement: read a local CIViC evidence YAML snapshot
    (CC0, fully redistributable) and return ActionabilityResult per
    (gene, variant) query. Until then any call raises NotImplementedError
    so callers know to wire NullActionabilityClient or StubActionabilityClient
    explicitly.

    This intentionally has NO httpx dependency — CIViC distribution is
    the local snapshot under knowledge_base/hosted/civic/, not a live API."""
    ...
    def lookup(self, query: ActionabilityQuery) -> ActionabilityResult | ActionabilityError:
        raise NotImplementedError(
            "SnapshotActionabilityClient lookup is Phase 2 — not yet wired. "
            "Use NullActionabilityClient or StubActionabilityClient for now."
        )
    ...
    def batch_lookup(
        ...
        raise NotImplementedError(
            "SnapshotActionabilityClient batch_lookup is Phase 2 — not yet wired."
        )
